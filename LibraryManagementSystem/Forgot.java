package LibraryManagementSystem;

import LibraryManagementSystem.ConnectToDatabase;

import java.awt.*;
import javax.swing.*;
import javax.swing.border.*;
import java.sql.*;
import java.awt.event.*;

public class Forgot extends JFrame implements ActionListener{

    private JPanel contentPane;
    private JTextField t1,t2,t3,t4,t5;
    private JButton b1,b2,b3;

    public static void main(String[] args) {

        new Forgot().setVisible(true);
    }

    public Forgot() {

        setBounds(200, 200, 620, 450);
        contentPane = new JPanel();
        contentPane.setBorder(new EmptyBorder(5, 5, 5, 5));
        setContentPane(contentPane);
        contentPane.setBackground(new Color(240, 228, 215));
        contentPane.setLayout(null);

        JLabel l1 = new JLabel("Username");
        l1.setFont(new Font("Verdana", Font.PLAIN, 13));
        l1.setBounds(109, 83, 87, 29);
        contentPane.add(l1);

        JLabel l2 = new JLabel("Name");
        l2.setFont(new Font("Verdana", Font.PLAIN, 13));
        l2.setBounds(109, 122, 75, 21);
        contentPane.add(l2);

        JLabel l3 = new JLabel("Your Security Question");
        l3.setFont(new Font("Verdana", Font.PLAIN, 13));
        l3.setBounds(109, 154, 156, 27);
        contentPane.add(l3);

        JLabel l4 = new JLabel("Answer");
        l4.setFont(new Font("Verdana", Font.PLAIN, 13));
        l4.setBounds(109, 192, 104, 21);
        contentPane.add(l4);

        JLabel l5 = new JLabel("Password");
        l5.setFont(new Font("Verdana", Font.PLAIN, 13));
        l5.setBounds(109, 224, 104, 21);
        contentPane.add(l5);

        t1 = new JTextField();
        t1.setFont(new Font("Verdana", Font.PLAIN, 13));
        t1.setForeground(new Color(51,51, 51));
        t1.setBounds(277, 88, 139, 20);
        contentPane.add(t1);
        t1.setColumns(10);

        t2 = new JTextField();
        t2.setEditable(false);
        t2.setFont(new Font("Verdana", Font.PLAIN, 13));
        t2.setForeground(new Color(51,51, 51));
        t2.setColumns(10);
        t2.setBounds(277, 123, 139, 20);
        contentPane.add(t2);

        t3 = new JTextField();
        t3.setEditable(false);
        t3.setFont(new Font("Verdana", Font.PLAIN, 12));
        t3.setForeground(new Color(51,51, 51));
        t3.setColumns(10);
        t3.setBounds(277, 161, 221, 20);
        contentPane.add(t3);

        t4 = new JTextField();
        t4.setFont(new Font("Verdana", Font.PLAIN, 13));
        t4.setForeground(new Color(51,51, 51));
        t4.setColumns(10);
        t4.setBounds(277, 193, 139, 20);
        contentPane.add(t4);

        t5 = new JTextField();
        t5.setEditable(false);
        t5.setFont(new Font("Verdana", Font.PLAIN, 13));
        t5.setForeground(new Color(51,51, 51));
        t5.setColumns(10);
        t5.setBounds(277, 225, 139, 20);
        contentPane.add(t5);

        b1 = new JButton("Search");
        b1.addActionListener(this);
        b1.setFont(new Font("Verdana", Font.PLAIN, 12));
        b1.setBounds(428, 83, 80, 29);
        b1.setBackground(new Color(255, 113, 113));
        b1.setBorder(new LineBorder(new Color(0,0,0), 1, true));
        b1.setForeground(Color.BLACK);
        contentPane.add(b1);

        b2 = new JButton("Retrieve");
        b2.addActionListener(this);
        b2.setFont(new Font("Verdana", Font.PLAIN, 12));
        b2.setBackground(new Color(255, 113, 113));
        b2.setBounds(426, 188, 85, 29);
        b2.setBorder(new LineBorder(new Color(0,0,0), 1, true));
        b2.setForeground(Color.BLACK);
        contentPane.add(b2);

        b3 = new JButton("Back");
        b3.addActionListener(this);
        b3.setFont(new Font("Verdana", Font.PLAIN, 13));
        b3.setBounds(233, 270, 101, 29);
        b3.setBorder(new LineBorder(new Color(0,0,0), 1, true));
        b3.setBackground(new Color(255, 113, 113));
        b3.setForeground(Color.BLACK);
        contentPane.add(b3);

        JPanel panel = new JPanel();
        panel.setBorder(new TitledBorder(new LineBorder(new Color(0,0,0), 2), "Forgot Password",
                TitledBorder.LEADING, TitledBorder.TOP, null, new Color(0,0,0)));
        panel.setBounds(47, 45, 508, 281);
        panel.setBackground(new Color(240, 228, 215));
        contentPane.add(panel);
    }

    public void actionPerformed(ActionEvent ae){
        try{
            ConnectToDatabase con = new ConnectToDatabase();
            if(ae.getSource() == b1){
                String sql = "select * from signup where username=?";
                PreparedStatement st = con.connect.prepareStatement(sql);

                st.setString(1, t1.getText());
                ResultSet rs = st.executeQuery();

                while (rs.next()) {
                    t2.setText(rs.getString("name"));
                    t3.setText(rs.getString("sq"));
                }

            }
            if(ae.getSource() == b2){
                String sql = "select * from signup where answer=?";
                PreparedStatement st = con.connect.prepareStatement(sql);

                st.setString(1, t4.getText());
                ResultSet rs = st.executeQuery();

                while (rs.next()) {
                    t5.setText(rs.getString("password"));
                }

            }
            if(ae.getSource() == b3){
                this.setVisible(false);
                new LoginUser().setVisible(true);

            }
        }catch(Exception e){

        }
    }

}
